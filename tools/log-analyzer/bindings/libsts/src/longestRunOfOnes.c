...
longest_run_of_ones LongestRunOfOnes(int n) {
  double pval, chi2, pi[7];
  int run, v_n_obs, N, i, j, K, M, V[7];
  unsigned int nu[7] = {0, 0, 0, 0, 0, 0, 0};
  longest_run_of_ones ret = {0};
  ret.passed = false; // just to make it more explicit
  if (n < 128) {
  ...
  if (stats[TEST_LONGEST_RUN]) {
    ...
    if (K == 3) {
      ...
      fprintf(stats[TEST_LONGEST_RUN], "\n\t\t %3d %3d %3d %3d %3d %3d  %3d ",
              nu[0], nu[1], nu[2], nu[3], nu[4], nu[5], nu[6]);
    }
