...
/** Array primitives **/
gen_array_remove_at(csi_t)
gen_array_remove_at(quant_index_t)

/** Sorting algorithms **/
gen_insert_sort(csi_t)
gen_insert_sort(quant_index_t)
gen_quick_sort(csi_t)

gen_sort_check(csi_t)
gen_sort_check(quant_index_t)

void
shift_array_left(void *arr, size_t size, size_t at, size_t shift_by, size_t elem_size)
{
	// TODO: Implements the function. Move data to new position and override garbage with zeroes
}

struct density *
get_density(csi_t *csis, size_t num_csi)
{
  struct density *d;
...
struct density *
compute_pdf(csi_t *csis, size_t num_csi)
{
  struct density *d;
...
struct density *
compute_cdf(struct density *pdf)
{
  struct density *d;
