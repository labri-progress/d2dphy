...
#define gen_array_remove_at(ty)                                 \
    ...
    {                                                           \
        if ((*num_elements) - 1 != index)                        \
          memcpy(&arr[index], &arr[index+1], (*num_elements - index - 1) * sizeof(ty)); \
...
extern void quant_index_t_remove_at(quant_index_t *arr, size_t *num_elements, size_t index);
...
/*
    return value:
        ...
        0 : in case of error
*/
extern unsigned char PHYSEC_quntification_get_level(
    csi_t rssi,
