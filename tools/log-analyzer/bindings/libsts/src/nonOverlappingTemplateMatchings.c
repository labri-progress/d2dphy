...
non_overlapping_template_matchings NonOverlappingTemplateMatchings(int m,
                                                                   int n) {
  non_overlapping_template_matchings ret = {0};
  ret.valid_run = false;
  ...
  if (stats[TEST_NONPERIODIC] &&
      (Wj = (unsigned int *)calloc(N, sizeof(unsigned int))) == NULL) {
    fprintf(stats[TEST_NONPERIODIC], "\tNONOVERLAPPING TEMPLATES TESTS ABORTED "
  ...
  if (((isNegative(lambda)) || (isZero(lambda))) ||
    ...
    if (stats[TEST_NONPERIODIC]) {
      fprintf(stats[TEST_NONPERIODIC],
              "\tNONOVERLAPPING TEMPLATES TESTS ABORTED "
              "DUE TO ONE OF THE FOLLOWING : \n");
    ...
    if (stats[TEST_NONPERIODIC]) {
      ...
      fprintf(stats[TEST_NONPERIODIC],
              "Template   W_1  W_2  W_3  W_4  W_5  W_6  W_7  W_8    Chi^2   "
              "P_value Assignment Index\n");
      fprintf(
          stats[TEST_NONPERIODIC],
          "------------------------------------------------------------------"
          "-------------------\n");
    ...
    for (jj = 0; jj < MIN(MAXNUMOFTEMPLATES, numOfTemplates[m]); jj++) {
      ...
      if (stats[TEST_NONPERIODIC] && isNegative(p_value) ||
          isGreaterThanOne(p_value))
        fprintf(stats[TEST_NONPERIODIC],
  ...
  if (fp != NULL)
    fclose(fp);
  ret.chi_squared = chi2;
  ret.p_value = p_value;
  ret.passed = p_value >= ALPHA;
  ret.valid_run = true;
  return ret;
}
