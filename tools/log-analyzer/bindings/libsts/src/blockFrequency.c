...
block_frenquency BlockFrequency(int M, int n) {
  int i, j, N, blockSum;
  ...
  if (results[TEST_BLOCK_FREQUENCY]) {
    fprintf(results[TEST_BLOCK_FREQUENCY], "%f\n", p_value);
    fflush(results[TEST_BLOCK_FREQUENCY]);
  }
  block_frenquency ret;
  ret.chi_squared = chi_squared;
  ret.p_value = p_value;
