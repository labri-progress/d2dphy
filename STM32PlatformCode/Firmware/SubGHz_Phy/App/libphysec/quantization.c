 ...
 */
int add_bits_to_key(uint8_t *key, size_t key_capacity_in_bits, size_t at, uint8_t *bitstring, size_t n_bits)
{
	if (at >= key_capacity_in_bits)
		return key_capacity_in_bits;
	size_t num_max = (at + n_bits > key_capacity_in_bits) ? key_capacity_in_bits : n_bits+at; 
...
/*!
 *	\brief	Remove the bit at position 'at' in the key by shifting the bits to the left
 */
...
// FIX: Relaxe properties of required number of csi to faster KeyGen in good wireless env conditions
//bool
//quant_is_enough_csi(quant_type_t qtype, size_t num_csi)
//{
//	// TODO: Regarding the quantization method, check if there is enough gathered csi to perform quantization
//	switch (qtype) {
//		case QUANT_SB_DIFF_LOSSY:
//			if (num_csi >= 256) // FIX: this is the best case (we don't really know how many csi will be dropped)
//				return true;
//			break;
//		case QUANT_SB_EXCURSION_LOSSY:
//			if (num_csi >= 512) // FIX: scale on 'm' 
//				return true;
//			break;
//		case QUANT_MB_EXCURSION_LOSSY:
//			if (num_csi >= 512) // FIX: scale on 'm' and 'num_quant_level'
//				return true;
//		case QUANT_MBR_LOSSLESS:
//			if (num_csi >= 128)	// minimal limit for worst case
//				return true;
//		case QUANT_MBE_LOSSY:
//			if (num_csi >= 192)	// minimal limit for worst case
//				return true;
//		default:
//			break;
//	}
 ...
 * \param[in, out]	key					key to remove bits from
 * \param[in]		key_size_in_bits	size of the key in bits
 * \param[in]		indexes				array of indexes to remove from the key
 * \param[in]		num_indexes			number of indexes in the array
 *
 * \returns the new size of the key in bits
 */
uint32_t
quant_dropp_csis(quant_lossy_params_t *qparams, uint8_t *key, size_t key_size_in_bits, quant_index_t *indexes, size_t num_indexes) 
{
	if (qparams->dynamic)
		return -1;
	...
	for (size_t i=num_indexes-1; i > 0; i--) {
		...
		for (int j=0; j<qparams->nbits_per_sample; j++) {
			...
			add_bit_to_key(key, key_size_in_bits+1, key_size_in_bits, 0); // fill with zero on right
		}
...
uint32_t
quant_retain_csis(quant_lossy_params_t *qparams, uint8_t *key, size_t key_size_in_bits, quant_index_t *indexes, size_t num_indexes) {
 ...
 */
void
csi_indexes_sort(quant_index_t *indexes, size_t num_indexes)
{
	size_t i, j;
	quant_index_t key;
 ...
 *
 * \param[in, out]	indexes				array of indexes to merge (not sorted)
 * \param[in, out]	actual_num_indexes	number of indexes in the array
 * \param[in]		indexes_capacity	allocated capacity of the `indexes` array
 * \param[in]		new_indexes			array of new indexes to merge (not sorted)
 * \param[in]		num_new_indexes		number of new indexes to merge
 */
uint32_t
quant_merge_csi_indexes(quant_index_t *indexes, size_t *actual_num_indexes, size_t indexes_capacity, quant_index_t *new_indexes, size_t num_new_indexes) 
{
...
uint32_t
quant_inter_csi_indexes(quant_index_t *indexes, size_t *actual_num_indexes, size_t indexes_capacity, quant_index_t *new_indexes, size_t num_new_indexes)
{
  	if (*actual_num_indexes + num_new_indexes > indexes_capacity)
		return 0;
...
//int quant_complement_csi_indexes(quant_index_t *indexes, size_t *actual_num_indexes, size_t indexes_capacity, size_t num_csi)
//{
//  if (num_csi - *actual_num_indexes > indexes_capacity)
//    return -1;
//
//  quant_index_t complement[800];  // FIX: fix with define MAX_QUANT_INDEXES (1600 / 2)
//
...
//    memset(&indexes[cur_num_indexes], 0, (*actual_num_indexes - cur_num_indexes) * sizeof(quant_index_t));
//  }
//  *actual_num_indexes = cur_num_indexes;
//  return cur_num_indexes;
//}

void quant_excursion_select_random_subset(quant_index_t*excursions, size_t *num_excursions)
{
	// TODO: Select a random subset of excursions to keep (need to define a limit on the number of excursion to keep)
}

//bool quant_sb_excursion_get_indexes(quant_excursion_index_t *excursions, size_t num_excursions, quant_index_t *indexes, size_t *num_indexes)
//{
...
//}
//
//bool quant_sb_excursion_reconstruct_excursions(quant_index_t *indexes, size_t num_indexes, quant_excursion_index_t *excursions, size_t *num_excursions)
//{
//	// TODO: Modify the `excursions` array to select the excursions matching the valid `indexes`
//	return false;
//}
...
int
quant_sb_lossless(csi_t *measures, 
						 size_t n_measures, 
						 uint8_t *key, 
						 size_t key_size)
{
...
int quant_sb_lossy(csi_t *measures,
                   size_t n_measures,
                   uint8_t *outkey,
                   size_t key_size,
                   quant_index_t *indexes,
                   size_t *num_indexes)
{
	int num_quantized = 0;
  float mean = rssi_mean(measures, n_measures);
...
int quant_mbe_lossy(csi_t *measures, size_t n_measures, float alpha, uint8_t *outkey, size_t key_size, quant_index_t *lossy_points, size_t *num_lossy)
{
  // function have been coded to only handle blockwise quant,
  // so we need some checks to ensure it works only for this case
  if (n_measures >  QUANT_MBE_WINDOW_SIZE) return 0;
  ...
  if (qunatification_level_nbr < 2) return 0; // don't quantize window if only one level
  if (qunatification_level_nbr % 2 != 0) return 0; // abort on non equiprobalistic bit distribution
  if (qunatification_level_nbr > 16) qunatification_level_nbr = 16;
  ...
  for(int i = 0; i < n_measures; i++){
      ...
      if(level > 0){
          level--;
          add_bits_to_key(outkey, key_capacity_in_bits, num_quantized, &level, gen_bits); 
          ...
          //}else{
          //    outkey[key_char_index] += level<<(8-nbr_of_generated_bits_by_char-gen_bits);
  ...
  PHYSEC_quntification_free_density(&density);
	...
	return num_quantized; // 8*key_char_index+nbr_of_generated_bits_by_char;  //num_quantized;
}

// This function is used by Alice before quantization to check whether the number
// of measurements she has will allow her to generate 128 bits, based on the
...
int quant_mbr_lossless(int16_t *measures, size_t n_measures, uint8_t *outkey, size_t key_size) {
      ...
      if (j == n_levels-1)
        ...
        hbound = min + interval * (j+1);	
			if (measures[i] >= lbound && measures[i] < hbound) {
				...
				add_bits_to_key(outkey, key_size*8, num_quantized, &tmp, nbits_per_level);
...
int quant_sb_diff_lossy(int16_t *measures, size_t n_measures, uint8_t *outkey, size_t key_size, quant_index_t *lossy_points, size_t *num_lossy)
{
...
int quant_adaptive(int16_t *measures, size_t n_measures, uint8_t *outkey, size_t key_size, quant_index_t *lossy_points, size_t *num_lossy )
{
	return -1;
}

//int quant_sb_excursion_get_excursions(int16_t *measures, size_t n_measures, quant_excursion_index_t *excursions, size_t *num_excursions, uint8_t m, float alpha, size_t key_size)
//{
...
//			// i = breaked_at-1;  // this is an optimization which is 
//								  // not present in the paper's algorithm
//								  // so its commented
//			continue;
//		}
//
...
//		excursions[num_actual_excursions].index = (i*2+m-1) / 2;	// (i + i_end) / 2
//		num_actual_excursions++;
//		i += m-1;
//	}	
//	if (num_actual_excursions > *num_excursions) {
//		// TODO: truncate excursions instead of returning an error (If there is still enough excursion to generate a key)
//		return -1;
//	}
//
...
//		return -1;
//
//	quant_excursion_select_random_subset(excursions, &num_actual_excursions);
//
...
//int quant_sb_excursion_get_valid_excursions_from_indexes(int16_t *measures, size_t n_measures, quant_index_t *excursions_indexes, size_t num_indexes, quant_excursion_index_t *excursions, size_t *num_excursions, uint8_t m, float alpha, size_t key_size)
//{
//	// TODO: Quantize only the measures corresponding excursion indexes, modify `excursion_indexes`, fill the `excursion` array
//	
...
//
//	if (num_indexes < key_size*8 || *num_excursions < key_size*8 || *num_excursions < num_indexes)
//		return -1;
//
//	size_t num_actual_excursions = 0;
...
//		if ((measures[cur_idx] <= q_plus && measures[cur_idx] >= q_minus)
...
//int quant_sb_excursion_quantize(quant_excursion_index_t *excursions, size_t num_excursions, uint8_t *outkey, size_t key_size)
//{
...
int quant_sb_excursion_quantize2(csi_t *measures, size_t n_measures, quant_index_t *indexes, size_t *num_indexes, uint8_t m, float alpha,  uint8_t *outkey, size_t key_size)
{
  if (n_measures < m)
    return -1;
...
int quant_mb_excursion_quantize2(quant_lossy_params_t *qparams, csi_t *measures, size_t n_measures, quant_index_t *indexes, size_t *num_indexes, uint8_t m, uint8_t *outkey, size_t key_size)
{
  ...
  for (size_t i=0; i<n_measures-m; i++) {
    ...
    if (breaked_at == -1) {
      ...
      for (size_t k=0; k<m; k++) {
        add_bits_to_key(outkey, key_capacity_in_bits, i*nbits_per_level, &tmp, nbits_per_level);
